...
void matchDescriptors(std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,
                      std::vector<cv::DMatch> &matches, std::string descriptorType_HOT_or_BIN, std::string matcherType, std::string selectorType)
{
    // configure matcher
    ...
    if (matcherType.compare("MAT_BF") == 0)
    {
        ...
        int normType = descriptorType_HOT_or_BIN.compare("DES_BINARY") == 0 ? cv::NORM_HAMMING : cv::NORM_L2;
    ...
    else if (matcherType.compare("MAT_FLANN") == 0)
    {
        ...
        if (descriptorType_HOT_or_BIN.compare("DES_HOG") == 0)
        {
            matcher = cv::FlannBasedMatcher::create();
        }
        else if (descriptorType_HOT_or_BIN.compare("DES_BINARY") == 0)
        {
